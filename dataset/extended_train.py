...
class Extended_Dataset(CFNL_TrainDataset):
    ...
    def __init__(self, cnf=None, mode="train", cache=False):
        ...
        self.transforms = tf = transforms.Compose([
            transforms.ToTensor(),
            transforms.Resize(self.input_size, antialias=False),
        ...
        self.crop_transforms = transforms.Compose([
            transforms.ToTensor(),
            transforms.Resize((crop_height,crop_width), antialias=False),
            transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
        ])
    ...
    def __getitem__(self, item):
        ...
        # for i in boxes_ind:
        #     try:
        #         tempdets.append[dets[i]]
        #         tempego.append[ego_id[i]]
        #     except:
        #         print("exception while reading ego")
        #         print(uuid)
        #         # tempego.append[dets[0]]
        #         # tempego.append[ego_id[0]]
        # dets=tempdets
        # ego_id=tempego
        # sanity check
