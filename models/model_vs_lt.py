...
models = {
	...
	'efficientnet_b0': (EfficientNet.from_pretrained('efficientnet-b0', num_classes=256), {'pretrained': False, 'norm_layer': FrozenBatchNorm2d},
...
class TripleChebyshev(nn.Module):
    
    def __init__(self, dim: int = 1) -> None:
        super(TripleChebyshev, self).__init__()
        self.dim = dim

    def forward(self, x1, x2):
        d = torch.max(torch.abs(x1.unsqueeze(1)-x2), dim=-1).values
        return torch.mean(d, dim=-1)
    
dist_dict = {
	'cosine': TripleCosineSimilarity,
	'euclid': TripleEuclidean,
	'chebyshev' : TripleChebyshev
}
...
class ExtendedLiteModel(BaseModel):
	...
	def __init__(self, cnf):
		...
		dist_str = cnf.model_opts.get("DISTANCE", "chebyshev")
		...
		if self.use_ego_crop:
			ego_backbone = self.cnf.model_opts.get('BACKBONE', 'efficientnet_b0')
		...
		if self.compute_nl:
			...
			self.bert_model = AlbertModel.from_pretrained(bert_ck)
	...
	def compute_nl_embeddings(self, seq):
		...
		bert_out = self.bert_model(tokens['input_ids'],
	...
	def train_loss(self, x):
		...
		if not self.compute_nl:
			nl_embs = torch.stack([pos, neg], dim=1)
